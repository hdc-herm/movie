...
function ar_main() {
	...
	//收集完日志 提交到的路径
//	var dest_path   = "http://192.168.90.46/servlet/LogServlet?";
...
//	/"href="+href+"&pathname="+pathname+
	//拼接访问地址 增加如上信息    url+当前资源名+网页标题+字符集+屏幕大小+颜色+语言+是否启用jvm+是否启用cookie+flash+ref+浏览器类型+uv+session
	dest=dest_path+"url=="+url+"$urlname=="+urlname+"$title=="+title+"$chset=="+charset+"$scr=="+screen+"$col=="+color+"$lg=="+language+"$je=="+jvm_enabled+"$ce=="+cookie_enabled+"$fv=="+flash_ver+"$ref=="+ref+"$uagent=="+agent+"$stat_uv=="+uv_id+"$stat_ss=="+stat_ss;
